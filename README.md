...
If you want to contribute or run the tool from source:
...
2. **Install dependencies:**
   Since this project has zero runtime dependencies, this just sets up the package environment.
   ```bash
   npm install
   ```
...
4. **Link for local development:**
   To use the `mcp-scan` command globally while pointing to your local source code:
   ```bash
   npm link
   mcp-scan --version
   ```
